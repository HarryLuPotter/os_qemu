...
load_icode(unsigned char *binary, size_t size) {
    ...
    tf->gpr.sp = USTACKTOP;
    tf->epc = elf->e_entry;
    //tf->status = (sstatus & ~SSTATUS_SPP) | SSTATUS_SPIE;   //SPP要置0表示中断返回要回到U态，SPIE是中断使能标志
...
//参数：name是新程序名字，len新程序名字长度，binary指向新程序的二进制码，size为二进制数据大小
int                                                                             //用一个新的程序替换当前进程的内存空间和上下文
...
found:
    ...
    {
        unhash_proc(proc);
        remove_links(proc);             //从进程链表里摘下去，切断与其他进程的联系
    }
    local_intr_restore(intr_flag);
...
static int
kernel_execve(const char *name, unsigned char *binary, size_t size) {       //加载名字叫name的用户程序到本进程开始执行
